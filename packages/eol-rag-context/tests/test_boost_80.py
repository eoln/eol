...
for module in ['magic', 'pypdf', 'docx', 'redis', 'redis.asyncio', 'redis.commands',
               ...
               'fastmcp.server', 'markdown', 'gitignore_parser', 'pypdf.PdfReader',
               'docx.Document']:
...
async def boost_document_processor():
    ...
    # Test _process_pdf
    with patch('eol.rag_context.document_processor.pypdf') as MockPypdf, \
...
async def boost_redis_client():
    ...
    with patch('eol.rag_context.redis_client.AsyncRedis') as MockAsync:
        ...
        mock_async.ft.return_value.search = AsyncMock(
            return_value=MagicMock(docs=[], total=0)
        )
...
async def boost_semantic_cache():
    ...
    redis.redis.hgetall = AsyncMock(return_value={
        "cache:1": json.dumps({"query": "q1", "response": "r1", "timestamp": 1000, "hit_count": 5}),
        "cache:2": json.dumps({"query": "q2", "response": "r2", "timestamp": 2000, "hit_count": 2}),
        "cache:3": json.dumps({"query": "q3", "response": "r3", "timestamp": 3000, "hit_count": 10}),
    })
...
async def boost_knowledge_graph():
    ...
    for i in range(10):
        ...
        if i > 0:
            rel = knowledge_graph.Relationship(f"e{i-1}", f"e{i}", knowledge_graph.RelationType.USES)
...
async def boost_server():
    ...
    with patch('eol.rag_context.server.FastMCP') as MockMCP:
        ...
        srv.indexer.index_file = AsyncMock(
            return_value=MagicMock(source_id="src", chunks=5)
        )
        ...
        srv.cache.get_optimization_report = AsyncMock(return_value={
            "hit_rate": 0.28,
            "recommendations": ["Increase similarity threshold"]
        })
        ...
        result = await srv.structured_query("query", filters={"type": "code"}, options={"boost": 2.0})
