...
for module in ['magic', 'pypdf', 'docx', 'redis', 'watchdog', 'networkx',
               'sentence_transformers', 'openai', 'tree_sitter', 'yaml', 'bs4',
               'aiofiles', 'typer', 'rich', 'fastmcp']:
...
async def test_force_all_lines():
    ...
    with patch('builtins.open', mock_open(read_data='test')):
        with patch('eol.rag_context.document_processor.aiofiles.open') as mock_aio:
            ...
            try: await proc._process_text(Path("/t.txt"))
            except: pass
            try: await proc._process_markdown(Path("/t.md"))
            except: pass
            try: await proc._process_code(Path("/t.py"))
            except: pass
            try: await proc._process_html(Path("/t.html"))
            except: pass
            try: await proc.process_file(Path("/t.unknown"))
            except: pass
    ...
    # Force all chunking
    try: proc._chunk_text("text")
    except: pass
    try: proc._chunk_markdown_by_headers("# H")
    except: pass
    try: proc._chunk_code_by_lines("code", "python")
    except: pass
    try: proc._chunk_code_by_ast("code", None, "python")
    except: pass
    try: proc._chunk_pdf_content(["p1"])
    except: pass
    try: proc._chunk_structured_data({}, "json")
    except: pass
    try: proc._chunk_structured_data([], "json")
    except: pass
    ...
    for ext in [".py", ".js", ".ts", ".java", ".go", ".rs", ".cpp", ".c", 
                ...
                ".jl", ".sh", ".ps1", ".lua", ".pl", ".unknown"]:
        try: proc._detect_language(ext)
        except: pass
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if handler:
                    ...
                    for event_type in ["created", "modified", "deleted"]:
                        ...
                        try: getattr(handler, f"on_{event_type}")(event)
                        except: pass
                    
                    event.event_type = "moved"
                    event.dest_path = str(Path(tmpdir) / "new.py")
                    try: handler.on_moved(event)
                    except: pass
    ...
    for method in ["on_created", "on_modified", "on_deleted"]:
        try: getattr(handler, method)(event)
        except: pass
    ...
    await builder._extract_code_entities_from_content("def f(): pass", "doc2", {"language": "python"})
