        ...
        if uncached_texts:
            ...
            for i in range(0, len(uncached_texts), self.max_batch_size):
                ...
                try:
                    ...
                    for text in batch:
                        try:
                            emb = await self.embedding_manager.get_embedding(text, use_cache=use_cache)
...
class BatchRedisClient:
    ...
    async def store_documents_batch(self, documents: List[VectorDocument]) -> int:
                ...
                for doc in batch:
                    ...
                    if doc.metadata:
                        for k, v in doc.metadata.items():
                            if v is not None and not k.startswith('embedding'):  # Skip binary fields
        ...
        if elapsed_time > 0:
            ...
            logger.info(f"Bulk stored {stored_count} documents in {elapsed_time:.1f}s ({rate:.1f} docs/sec)")
    ...
    async def bulk_vector_search(
        ...
        search_tasks = [
            self.redis_store.vector_search(
                query, 
                embedding_manager, 
                k=k, 
                query_embedding=emb
