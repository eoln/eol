...
class ParallelFileScanner:
    ...
    def _prioritize_files(self, files: List[Path]) -> List[Path]:
        ...
        def priority_key(file_path: Path) -> tuple:
            ...
            if any(pattern.lower().replace('*', '') in name_lower 
...
class ParallelIndexer(DocumentIndexer):
    ...
    async def index_folder_parallel(
        ...
        logger.info(f"Starting parallel indexing of {folder_path} with {self.parallel_config.max_document_workers} workers")
        ...
        async for batch in self.scanner.scan_repository(
            ...
            if (self.current_checkpoint.completed_files % 
                self.parallel_config.checkpoint_interval == 0):
        ...
        try:
            ...
            if data:
                ...
                self.current_checkpoint.processed_files = set(
                    data.get('processed_files', '').split(',') if data.get('processed_files') else []
                )
