...
class TestMCPServer:
    ...
    async def test_query_knowledge_graph_tool(self, rag_server):
        ...
        rag_server.components.graph.query_subgraph = AsyncMock(return_value=mock_subgraph)
    ...
    async def test_optimize_context_tool(self, rag_server):
        ...
        rag_server.components.cache.get_optimization_report = AsyncMock(return_value=mock_report)
    ...
    async def test_list_sources_resource(self, rag_server):
        ...
        rag_server.components.indexer.list_sources = AsyncMock(return_value=mock_sources)
    ...
    async def test_stats_resource(self, rag_server):
        """Test stats resource."""
        # Mock stats
        mock_stats = {"documents_indexed": 100, "cache_hit_rate": 0.31, "entities": 50}
        rag_server.components.indexer.get_stats = MagicMock(return_value={"documents_indexed": 100})
        rag_server.components.cache.get_stats = MagicMock(return_value={"hit_rate": 0.31})
        rag_server.components.graph.get_graph_stats = MagicMock(return_value={"entity_count": 50})
    ...
    async def test_error_handling(self, rag_server):
        ...
        # Mock error
        rag_server.components.indexer.index_folder = AsyncMock(side_effect=Exception("Test error"))
    ...
    def test_tool_registration(self, rag_server):
        ...
        tool_calls = [
            call[0][0] if call[0] else None for call in rag_server.mcp.tool.call_args_list
        ]
    ...
    def test_resource_registration(self, rag_server):
        ...
        resource_calls = [
            call[0][0] if call[0] else None for call in rag_server.mcp.resource.call_args_list
        ]
...
class TestServerRun:
    ...
    def test_run_method(self):
        """Test the run method."""
        with (
            patch("eol.rag_context.server.RAGComponents"),
