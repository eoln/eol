...
class CodeAssistant:
    ...
    async def find_implementations(self, entity: str) -> List[Dict[str, Any]]:
        ...
        graph = await self.server.query_knowledge_graph(
            entity,
            max_depth=2
        )
    ...
    async def suggest_improvements(self, code: str) -> List[str]:
        ...
        for practice in practices:
            suggestions.append(
                f"Best practice: {practice['content'][:150]}"
            )
                ...
                if user_input.startswith("find "):
                    ...
                    if implementations:
                        ...
                        for impl in implementations:
                            print(f"  📄 {impl['name']} ({impl['type']}) - {impl['file']}:{impl['line']}")
                    else:
