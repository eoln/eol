...
for module in ['redis', 'redis.asyncio', 'redis.commands', 'redis.commands.search',
               ...
               'docx', 'aiofiles', 'aiofiles.os', 'tree_sitter', 'tree_sitter_python',
               'tree_sitter_javascript', 'tree_sitter_typescript', 'tree_sitter_go',
               'tree_sitter_rust', 'tree_sitter_cpp', 'tree_sitter_c', 'tree_sitter_java',
               'tree_sitter_csharp', 'tree_sitter_ruby', 'tree_sitter_php', 'yaml', 'bs4',
...
class TestIndexer80:
    ...
    async def test_indexer_complete(self):
        ...
        emb.get_embeddings = AsyncMock(side_effect=lambda texts, **kwargs: np.random.randn(len(texts), 768))
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            idx.scanner.scan_folder = AsyncMock(return_value=[
...
class TestServer80:
    ...
    async def test_server_complete(self):
        ...
        with patch('eol.rag_context.server.FastMCP') as MockMCP:
            ...
            srv.indexer.index_folder = AsyncMock(return_value=MagicMock(
                source_id="src", file_count=10, total_chunks=50, errors=[]
            ))
            srv.indexer.index_file = AsyncMock(return_value=MagicMock(
                source_id="src", chunks=5
            ))
            ...
            srv.redis.vector_search = AsyncMock(return_value=[
                MagicMock(content="Result", metadata={"score": 0.9})
            ])
            ...
            srv.graph.query_subgraph = AsyncMock(return_value={
                "entities": [{"id": "e1"}],
                "relationships": []
            })
            ...
            srv.cache.get_optimization_report = AsyncMock(return_value={
                "hit_rate": 0.28,
                "recommendations": ["Increase threshold"]
            })
            ...
            srv.redis.get_context = AsyncMock(return_value=[
                MagicMock(content="Context", metadata={})
            ])
            ...
            srv.redis.vector_search = AsyncMock(return_value=[
                MagicMock(content="Result", metadata={"type": "code"})
            ])
...
class TestEmbeddings80:
    ...
    async def test_embeddings_complete(self):
        ...
        cfg_openai = config.EmbeddingConfig(
            provider="openai",
            openai_api_key="key",
            dimension=1536
        )
