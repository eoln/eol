...
def test_knowledge_graph():
    ...
    try:
        # Extract entities
        loop.run_until_complete(builder._extract_markdown_entities("# Title\n`code`", "doc1", {}))
        loop.run_until_complete(
            builder._extract_code_entities_from_content(
                "def func(): pass", "doc2", {"language": "python"}
            )
        )
        loop.run_until_complete(
            builder._extract_structured_entities({"api": "test"}, "doc3", {"doc_type": "json"})
        )
...
def test_server():
    ...
    with patch("eol.rag_context.server.FastMCP") as MockMCP:
        ...
        srv.processor.process_file = AsyncMock(
            return_value=document_processor.ProcessedDocument(
                file_path=Path("/test.md"), content="Test", doc_type="markdown", chunks=[]
            )
        )
...
def test_main():
    ...
    # Test help
    with (
        patch("eol.rag_context.main.sys.argv", ["prog", "--help"]),
