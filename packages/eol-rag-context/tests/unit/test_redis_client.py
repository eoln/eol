...
async def test_store_document_prefix_mapping():
    ...
    chunk_doc = redis_client.VectorDocument(
        ...
        embedding=np.random.rand(384).astype(np.float32),
        hierarchy_level=3,
        metadata={"created_at": 1234567890, "position": 1, "doc_type": "code", "language": "python"}
    )
...
async def test_vector_search_with_filters():
    ...
    mock_results = [
        ("doc1", 0.9, {"content": "Test 1", "metadata": {"type": "test"}, "parent": None, "children": []}),
        ("doc2", 0.8, {"content": "Test 2", "metadata": {"type": "test"}, "parent": None, "children": []})
    ]
