...
class TestBatchEmbeddingManager:
    ...
    async def test_get_embeddings_batch_with_cache(self, batch_embedder):
        ...
        batch_embedder.embedding_manager.get_embeddings.assert_called_once_with(["new_text"], use_cache=True)
...
class TestBatchRedisClient:
    ...
    @pytest.mark.asyncio
    async def test_store_documents_batch_vadd_failure(self, batch_client, sample_documents, mock_redis_store):
    ...
    @pytest.mark.asyncio
    async def test_store_documents_batch_pipeline_failure(self, batch_client, sample_documents, mock_redis_store):
        """Test fallback to individual storage on pipeline failure."""
        # Mock pipeline failure
        mock_redis_store.redis.pipeline.return_value.execute.side_effect = Exception("Pipeline failed")
...
class TestStreamingProcessor:
    ...
    async def test_process_large_file_stream(self, processor, tmp_path):
        ...
        content = "This is sentence one. This is sentence two. This is sentence three. Final sentence."
