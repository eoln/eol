...
async def test_openai_provider():
    ...
    with patch("eol.rag_context.embeddings.AsyncOpenAI") as MockOpenAI:
        ...
        mock_response.data = [MagicMock(embedding=np.random.randn(1536).tolist())]
        ...
        # Mock batch response
        mock_response.data = [
            MagicMock(embedding=np.random.randn(1536).tolist()) for _ in range(len(texts))
        ]
...
async def test_embedding_dimension_validation():
    ...
    config_obj = config.EmbeddingConfig(
        provider="sentence_transformers", model_name="custom-model", dimension=512
    )
