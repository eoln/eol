...
class IndexingTaskInfo:
    ...
    def estimated_completion_time(self) -> Optional[float]:
        ...
        if (self.status != TaskStatus.RUNNING or 
            self.files_per_second <= 0 or 
    ...
    async def _load_task_info(self, task_id: str) -> Optional[IndexingTaskInfo]:
        ...
        try:
            ...
            if data:
                ...
                computed_fields = ['progress_percentage', 'files_per_second', 
                                 'estimated_completion_time', 'elapsed_time']
