...
async def test_nonblocking_tools():
    ...
    try:
        ...
        result = await server.index_directory(
            path=test_data_path, description="Integration test indexing"
        )
...
async def test_system_responsiveness():
    ...
    try:
        ...
        result = await server.index_directory(
            path=test_data_path, description="Responsiveness test"
        )
        task_id = result.get("task_id")
        ...
        if task_id:
            ...
            print(
                f"⏱️  All {len(operations)} operations completed in {end_time - start_time:.3f} seconds"
            )
            ...
            if failed_count == 0:
                ...
                print(
                    f"⚠️  {failed_count}/{len(operations)} operations failed during concurrent execution"
                )
